...
public class PeopleSprinkler {
	...
	public void initializeResidence(int numberOfPeople, Map map,
		...
		for (Cell c : map.canadaCells) {
			double des = map.totalDes.field[c.x][c.y];
		...
		while (totalHousehold > 0) {
			for (int i = 0; i < regionList.size(); ++i) {
				...
				// sprinkle it
				if (region.remainPopulation > 0) {

					ArrayList<Cell> cellOfInterest = popMap
							.get(region.regionCode);
					if (numberOfPeople > region.remainPopulation)
						numberOfPeople = region.remainPopulation;
					if (cellOfInterest != null) {
						this.sprinklePeople(numberOfPeople, socialWeight,
					...
					totalHousehold -= numberOfPeople;
					region.remainPopulation -= numberOfPeople;
				}
	...
	private void sprinklePeople(int numberOfPeople, double socialWeight,
	 ...
	 */
	public int chooseStochasticallyFromCumulativeProbs(double[] cumulProbs,
			double total, MersenneTwisterFast rand) {
		int val = Arrays.binarySearch(cumulProbs, rand.nextDouble() * total);
		if (val < 0) {
			val = -(val + 1); // Arrays.binarySearch(...) returns (-(insertion
								// point) - 1) if the key isn't found
		}
	 ...
	 */
	public Double2D calcDesirabilityBounds(Map map) {
...
class Region implements Comparable<Region> {
	...
	public Region(int regionCode, int numberOfCells, int population) {
		this.regionCode = regionCode;
		this.numberOfCells = numberOfCells;
		this.population = population;
		this.remainPopulation = population;
	}
