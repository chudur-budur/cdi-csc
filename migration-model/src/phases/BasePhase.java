...
public class BasePhase extends SimState implements ECInterface
{
	...
	public DoubleGrid2D smoothedTargetPopGrid; // smoothed population grid
	public DoubleGrid2D zscoreSmoothedTargetPopGrid; // A z-score version of the smoothed population
	

	public double getTempDesCoefficient() {
		return parameters.tempCoeff;
	}

	public void setTempDesCoefficient(double val) {
		if(COEFF_LOWER_BOUND<=val&&val<=COEFF_UPPER_BOUND)
			parameters.tempCoeff = val;	
	}
	...
	public double getPortDesCoefficient() {
		return parameters.portCoeff;
	}

	public void setPortDesCoefficient(double val) {
		if(COEFF_LOWER_BOUND<=val&&val<=COEFF_UPPER_BOUND)
			parameters.portCoeff = val;
	}
	...
	public double getRiverDesCoefficient() {
		return parameters.riverCoeff;
	}

	public void setRiverDesCoefficient(double val) {
		if(COEFF_LOWER_BOUND<=val&&val<=COEFF_UPPER_BOUND)
			parameters.riverCoeff = val;
	}
	...
	public double getElevDesCoefficient() {
		return parameters.elevCoeff;
	}

	public void setElevDesCoefficient(double val) {
		if(COEFF_LOWER_BOUND<=val&&val<=COEFF_UPPER_BOUND)
			parameters.elevCoeff = val;
	}
    ...
    {
        // Create a smoothed version of the target population grid
    ...
    /**
     * Returns an array containing the empirical population distribution,
     * but only for the grid cells we care about (i.e. Canada).
